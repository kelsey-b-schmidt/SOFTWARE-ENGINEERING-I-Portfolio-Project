    ...
    for i in range(9):
        ...
        for i in range(9):
            ...
            for k in range(0,row):
                choices = [x for x in choices if x != table[k][column]]

            # along same row from right to current position
            for k in range(0,column):
                choices = [x for x in choices if x != table[row][k]]

            # 2nd row of blocks
            # 1st column
            if column % 3 == 0 and (row+2) % 3 == 0:
                for k in range((column+1), (column+3)):
                    choices = [x for x in choices if x != table[row-1][k]]
            # 2nd column
            ...
            if column % 3 == 0 and (row+1) % 3 == 0:
                for k in range((row-2), row):
                    for l in range((column+1), (column+3)):
                        choices = [x for x in choices if x != table[k][l]]
...
def board_maker(table):
    return tabulate(table, tablefmt='html')

#print(board_maker(table))